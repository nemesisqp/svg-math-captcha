...
function Bitmap4BBPContext(bitmap) {
    ...
    this._fillColor = 0x000000ff;   // the real int holding the RGBA value
    this._fillStyle_text = "white"; // the text version set by using the fillStyle setter.
    this._strokeColor = 0x000000ff;
    this._strokeStyle_text = "white";
    ...
    this.compositePixel = function (x, y, new_int) {
        ...
        var final_int = compositePixel(new_int, old_int, this.mode);
    ...
    this.stroke = function () {
        ...
        lines.forEach(function (line) {
            // console.log('ctx._strokeColor', ctx._strokeColor);
    ...
    this.clearRect = function (x, y, w, h) {
        for (var i = x; i < x + w; i++) {
            for (var j = y; j < y + h; j++) {
                this.setPixeli32(i, j, 0x000000ff);
            }
        }
    };
...
function compositePixel(src, dst, omode) {
    if (omode == 'REPLACE') {
        return src;
    }
