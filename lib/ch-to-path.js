...
function doSnap(path) {
    let snapStrength = random.int(18, 30);
    let snapDistance = random.int(14, 29);
    let snapX = random.int(2, 7);
    let snapY = random.int(2, 7);
    let i;
...
module.exports = function getPath(text, options) {
    ...
    let path = font.getPath(ch, left, baseline, fontSize, {kerning: true});
    doSnap(path);
